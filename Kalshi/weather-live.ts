#!/usr/bin/env bun
/**
 * Satriales Weather Trading Simulator v2 — Ladder Strategy
 *
 * Scans Polymarket weather markets, builds ladders across mispriced
 * brackets using multi-model ensemble forecasts (Open-Meteo + ECMWF +
 * GFS + NOAA), overweights cheap brackets for asymmetric payoffs.
 *
 * Usage:
 *   bun run weather-live.ts --instant              # quick test
 *   bun run weather-live.ts --balance 500          # live mode
 *   bun run weather-live.ts --min-edge 0.20 --ladder-budget 20
 */
...
const RESUME = !FRESH_START; // resume by default unless --fresh
...
function printBanner() {
  console.log(`
${c.cyan}${c.bold}  ╔═══════════════════════════════════════════╗
  ║   SATRIALES WEATHER TRADING SIMULATOR     ║
  ...
  ${c.dim}Models:${c.reset}            Open-Meteo + ECMWF + GFS + NOAA (US)
  ${c.dim}Max model spread:${c.reset}  ±${MAX_SPREAD.toFixed(0)}°F (skip when models disagree more)
  ${c.dim}Sizing:${c.reset}            Edge-weighted, 2x bonus on brackets ≤$0.10
  ${c.dim}Resolution:${c.reset}        ${INSTANT_RESOLVE ? "Instant (needs past dates)" : "Real temps from Open-Meteo archive API"}
...
function printDashboard(sim: WeatherSimulator) {
  ...
  console.log(`${c.bold}  DASHBOARD${c.reset}  ${c.dim}${now}${c.reset}  ${c.dim}Scan #${s.scansCompleted}${c.reset}\n`);
  ...
  // Recent results grouped by ladder
  if (s.closedPositions.length > 0) {
    // Group closed positions by ladder and show last 5 ladders
  ...
  const avgEdge = s.closedPositions.length > 0
    ? (s.closedPositions.reduce((sum, p) => sum + p.edge, 0) / s.closedPositions.length * 100).toFixed(1)
    : "—";
  ...
  console.log(`  ${pad("Avg entry edge:", 22)} ${c.white}${avgEdge}%${c.reset}`);
