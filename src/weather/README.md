...
| File | Purpose |
|------|---------|
| **`WeatherEnsemble.ts`** | Multi-model forecast ensemble. Queries Open-Meteo (GFS, ICON, ECMWF seamless) + NOAA weather.gov. Returns `{date, highF_mean, highF_sigma, members[]}`. |
...
Where μ = ensemble mean, σ = ensemble std dev. Implemented in `WeatherEnsemble.bracketProbability()`.
